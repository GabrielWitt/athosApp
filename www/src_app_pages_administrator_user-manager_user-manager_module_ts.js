...
let ProfileDetailComponent = class ProfileDetailComponent {
  ...
  emailListener(e) {
    ...
    if (!this.ValidateEmail(this.myCurrentUser.email)) {
      this.alertEmail = 'Email inválido';
    } else {
      this.alertEmail = '';
    }
  }
  ...
  sendData() {
    if (!this.myCurrentUser.name) {
      this.missingData = 'Primer nombre';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (this.myCurrentUser.name.length > 1) {
      this.missingData = 'Primer nombre completo';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (!this.myCurrentUser.secondName) {
      this.missingData = 'Segundo nombre';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (this.myCurrentUser.secondName.length > 2) {
      this.missingData = 'Número de identificación completo';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (!this.myCurrentUser.lastName) {
      this.missingData = 'Primer apellido';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (this.myCurrentUser.lastName.length > 2) {
      this.missingData = 'Apellido completo';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (!this.myCurrentUser.CI) {
      this.missingData = 'Número de identificación';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (this.myCurrentUser.CI.length > 4) {
      this.missingData = 'Número de identificación completo';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (!this.myCurrentUser.email) {
      this.missingData = 'Email';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (!this.ValidateEmail(this.myCurrentUser.email)) {
      this.missingData = 'Email correcto';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (!this.myCurrentUser.phonePersonal) {
      this.missingData = 'Telefono Principal';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }
    ...
    if (phonePersonal.length > 4) {
      this.missingData = 'Telefono Principal completo';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    }

    if (!this.myCurrentUser.birthDate) {
      this.missingData = 'Fecha de Nacimiento';
      this.show('Información Incompleta', 'Falta dato: ' + this.missingData);
      return;
    } else {
      this.missingData = '';
    ...
    this.show('USUARIO', this.user ? 'Datos de ' + this.user.name + ' ' + this.user.lastName + ' han sido actualizados' : 'Nuevo usuario agregado', 'OK');
    this.loading = false;
    this.modal.dismiss({
      action: 'reload'
    });
